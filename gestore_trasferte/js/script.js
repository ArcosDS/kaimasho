...
function aggiornaMissioni() {
    ...
    rows.forEach((row) => {
        ...
        if (missione) {
            ...
            row.querySelector("td:nth-child(8)").textContent = missione.costo.toFixed(2);
...
document.addEventListener("DOMContentLoaded", function () {
	...
	function aggiungiMissioneAllaTabella(id, dataInizio, giornoInizio, dataFine, giornoFine, descrizione, numeroGiorni, costo, trasfertaEstera) {
		...
		newRow.innerHTML = `
			...
			<td><input readonly oninput="updateMissionRow(${id})" class="editable" data-id="${id}" data-field="costo" type="number" step="0.01" value="${costo}"></td>
